...
function CheckList({setRank}) {
  ...
  const baseUrl = 'http://api.aladhan.com/v1/timingsByAddress?adress=tunisia';
  ...
  const Salat = [
    {name: 'El-Sobh', date: date1, updateDate:setDate1, Salat:dateSalat1,rank:rank1, setrank: setRank1},
    {name: 'El-Dhohr', date: date2, updateDate: setDate2, Salat:dateSalat2,rank:rank2, setrank: setRank2},
    {name: 'El-Asser', date: date3, updateDate: setDate3, Salat:dateSalat3,rank:rank3, setrank: setRank3},
  ...
  return (
    <Block
      ...
      padding={20}
      marginBottom={20}>
        {loading? <Text>loading data ...</Text> : <View>
        <Text align='center' paddingBottom={5} semibold>confirm your daily Salat </Text>
        <View  style={{flex: 1, flexDirection: 'row', alignContent:"space-between"}}>
        <Text style={{paddingRight:30}}>name</Text>
        <Text style={{paddingRight:30,alignSelf:"flex-end"}}>salat time</Text>
        <Text style={{ alignSelf:"flex-end"}}> time checked</Text>
      </View>
      {Salat.map(el => {return(<View key={el.name} style={{flex: 1, flexDirection: 'row', alignContent:"space-between"}}>
        <Checkbox marginRight={10} updateDate={el.updateDate} setrank={el.setrank} />
        <Text style={{paddingRight:30}}>{el.name}</Text>
        <Text style={{paddingRight:30,alignSelf:"flex-end"}}>{el.Salat}</Text>
